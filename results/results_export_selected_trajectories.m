...
%RESULTS_EXPORT_SELECTED Export some trajectories/segments of interest
    ...
    [labels_data, full_tags] = g_trajectories.read_tags(constants.FULL_TRAJECTORIES_TAGS_PATH);
    ...
    if ~isempty(idx)
        ...
        for i = 1:length(pos)
           ...
           set(gcf, 'Color', 'w');
           fn = fullfile(constants.OUTPUT_DIR, sprintf('/trajectory_s%d_d%d_t%d.eps', g_trajectories.items(pos(i)).set, g_trajectories.items(pos(i)).session, g_trajectories.items(pos(i)).track));
           export_fig(fn);           
        end       
    end
    ...
    [labels_data, segment_tags] = g_segments.read_tags(constants.DEFAULT_TAGS_PATH);
    ...
    if ~isempty(idx)
        ...
        for i = 1:length(pos)
           ...
           set(gcf, 'Color', 'w');
           export_fig(fullfile(constants.OUTPUT_DIR, ...
