...
%                1 = .fig file, 2 = .eps file, 3 = export_fig .eps file
%            generates one of the following:
%                C:\myfig_1.fig OR C:\myfig_1.eps OR C:\myfig_1.eps (paper quality)
...
function export_figure(flag, get_current_figure, get_path, get_name)
    ...
    if flag == 1
       % saveas(get_current_figure, strcat(get_path,get_name,'.fig')); %default export format
        ...
        while (1)
            prompt = sprintf('Exporting...: \n1. .fig format \n2. .eps format \n3. High quality .eps format (requires external library) \nChoice: ');
            ...
            elseif user_input == 3
                export_fig(fullfile(get_path, strcat(get_name,'.eps')));
                break;
            else
                prompt = 'Wrong input'
            end
